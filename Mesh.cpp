...
Mesh::Mesh(const char* objFile, Microsoft::WRL::ComPtr<ID3D12Device> device)
{
	...
	vbView.SizeInBytes = sizeof(Vertex) * verts.size();
	vbView.BufferLocation = vb->GetGPUVirtualAddress();
	ibView.Format = DXGI_FORMAT_R32_UINT;
	ibView.SizeInBytes = sizeof(unsigned int) * indices.size();
...
//
//
//void Mesh::SetBuffersAndDraw(Microsoft::WRL::ComPtr<ID3D12Resource> context)
//{
...
D3D12_VERTEX_BUFFER_VIEW Mesh::GetVertBufferView()
{
	return vbView;
}
